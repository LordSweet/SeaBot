...
namespace SeaBotGUI.Debug
{
    ...
    public static class NeededContractorAlgo
    {
        public static BindingList<BestAlgo> Algos = new BindingList<BestAlgo>();
        private static Thread BestAlgoThread;
        ...
        public static void UpdateBestStrategy()
        {
            ...
            while (true)
            {
                ...
                if ((DateTime.Now - _lastupdatedTime).TotalSeconds >= 1)
                {
                    ...
                    _lastupdatedTime = DateTime.Now;
                    var list = SeaBotCore.Utils.AutoTools.NeededItemsForContractor();
                    ...
                    if (Form1.instance.NeededContractorAlgoGrid.InvokeRequired)
                    {

                        MethodInvoker meth = () =>
                        {
                            foreach (DataGridViewTextBoxColumn clmn in Form1.instance.NeededContractorAlgoGrid.Columns)
                            {
                                clmn.AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                                clmn.Resizable = DataGridViewTriState.False;
                            }
                            ...
                            foreach (var bld in bestalgolist)
                            {
                                if (NeededContractorAlgo.Algos.Where(n => n.ID == bld.ID).FirstOrDefault() == null)
                                {
                                    Algos.Add(bld);
                                }
                                else
                            ...
                            Form1.instance.NeededContractorAlgoGrid.Refresh();
                            Form1.instance.NeededContractorAlgoGrid.Update();
                        };

                        Form1.instance.NeededContractorAlgoGrid.BeginInvoke(meth);
